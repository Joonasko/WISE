...
def generate_example_netcdf(output_file):
    ...
    precip_data = np.random.uniform(0, 0.2, size=(168, 5, 5))
    ...
    with nc.Dataset(output_file, "w") as dataset:
        ...
        temp_var = dataset.createVariable(
            "temperature", "f4", ("time", "point", "point")
        )
        relhum_var = dataset.createVariable(
            "relative_humidity", "f4", ("time", "point", "point")
        )
        ws_var = dataset.createVariable("wind_speed", "f4", ("time", "point", "point"))
        wd_var = dataset.createVariable(
            "wind_direction", "f4", ("time", "point", "point")
        )
        precip_var = dataset.createVariable(
            "precipitation", "f4", ("time", "point", "point")
        )
